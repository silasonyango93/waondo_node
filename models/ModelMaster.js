      ...
      return new Promise(function(resolve, reject) {
		  ...
		  con.query('INSERT INTO ' + tableName + ' SET ?', jsonObject_, function (err, result) {
            if (err){reject(err);}
				   ...
				   var returned_value_={success:true, message:"Record inserted succesfully.",recordId:result.insertId};
...
*/	
	static inverted_two_table_inner_join(TableOne,TableTwo,JoiningKey,SearchColumn,SearchValue) {
	return new Promise(function(resolve, reject) {	

        con.query('SELECT * FROM '+TableOne+' INNER JOIN '+TableTwo+' ON '+TableOne+'.'+JoiningKey+' = '+TableTwo+'.'+JoiningKey+' WHERE '+TableOne+'.'+SearchColumn+'= '+ mysql.escape(SearchValue),function (err, result) {
            if (err){reject(err);}
                   else {
...
*/	
	...
	return new Promise(function(resolve, reject) {	
        ...
        con.query('SELECT * FROM academic_class_levels INNER JOIN classes ON academic_class_levels.AcademicClassLevelId=classes.AcademicClassLevelId INNER JOIN students ON students.ClassId=classes.ClassId INNER JOIN primary_results_table ON students.AdmissionNo=primary_results_table.AdmissionNo WHERE academic_class_levels.AcademicClassLevelId='+ mysql.escape(AcademicClassLevelId)+' AND primary_results_table.ExamId='+ mysql.escape(ExamId)+' ORDER BY primary_results_table.MEAN DESC;',function (err, result) {
            if (err){reject(err);}
                   else {
